...
gantt
    ...
    Dify AI 知识库问答        :p2a, 2025-08, 2w
    AI 文章摘要（构建时）      :p2b, 2025-08, 2w
...
| 序号 | 功能 | 预估工时 | 依赖 | 状态 |
|------|------|---------|------|------|
| 2.1 | Dify AI 知识库问答 | 2 天 | Dify Cloud 账号 | ⏳ 待开始 |
| 2.2 | AI 文章摘要（构建时） | 3 天 | LLM API Key（CI 环境变量） | ✅ 已完成 |
| 2.3 | 文章底部 Mermaid 思维导图 | 2 天 | 构建时脚本 | ⏳ 待开始 |

**交付物**：
- `/ask-ai/` 页面（Dify 嵌入式 AI 问答）
- 文章页顶部 AI 摘要卡片
- 文章页底部自动生成的思维导图

**Phase 2.1 详细步骤（Dify 集成）**：
1. 注册 Dify Cloud 账号
2. 创建知识库 → 导入博客文章
3. 创建聊天助手应用 → 配置 Prompt
4. 获取嵌入式 iframe 代码
5. 在博客中创建 `/ask-ai/` 页面，嵌入 iframe
6. 可选：在侧边栏添加浮窗入口

**Phase 2.2 详细步骤（AI 摘要）**：
